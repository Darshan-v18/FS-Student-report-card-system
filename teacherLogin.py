...
def validate():
    ...
    with open('teachers.csv') as f:
        ...
        for row in reader:
            newList.append(row)
            if (a==newList[count][2] and b==newList[count][3]):
                flag = 1
                break
            count += 1
...
Label(root,text="Faculty Login", fg="RoyalBlue2",font="comicsansms 10 bold", pady=5).place(x=250,y=50)
...
# Button(text="<-",command=back).place(x=1,y=1);
root.mainloop();